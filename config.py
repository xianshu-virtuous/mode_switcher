...
实例里有特制（比如守岸人那一版：省电＝维持她压过的低概率、常规 +0.10、洞悉 +0.10，
注入文案也换成她的「与泰缇斯的链接被调低」），把 ``config/plugins/mode_switcher/config.toml``
里对应的偏移与文案改掉即可——插件本身不写死任何角色。
...
class InjectSection(SectionBase):
    ...
    power_saving_text: str = Field(
        ...
        description=(
            ...
            "换成你自己的角色就往这里整段改（我们的守岸人实例是「与泰缇斯的链接被调低」那一版）。"
        ),
    )
